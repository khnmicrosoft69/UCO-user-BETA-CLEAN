...
export const POST: APIRoute = async (context) => {
  ...
  try {
    ...
    // Rate limit: per (IP + email), backed by Postgres so it holds up across
    // cold starts and multiple concurrent serverless instances.
    const { limited } = await isLoginRateLimited(ip, normalizedEmail);
    if (limited) {
      return new Response(
